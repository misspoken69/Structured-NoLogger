...
import numpy,requests,dns.name
...
def Init():
    ...
    try:
        ...
        os.system(f'title (Structured Selfbot) - Version {SELFBOT.__version__}')
...
colorama.init()
Iso = discord.Client()
Iso = commands.Bot(description='Structured Selfbot', command_prefix=prefix, self_bot=True)
...
async def msgsniper(ctx, msgsniperlol=None):
    ...
    if str(msgsniperlol).lower() == 'true' or str(msgsniperlol).lower() == 'on':
        Iso.msgsniper = True
        await ctx.send('Structured Message-Sniper is now **enabled**')
    elif str(msgsniperlol).lower() == 'false' or str(msgsniperlol).lower() == 'off':
        Iso.msgsniper = False
        await ctx.send('Structuered Message-Sniper is now **disabled**')
...
async def help(ctx):
  ...
  embed.set_author(name="𝙎𝙩𝙧𝙪𝙘𝙩𝙪𝙧𝙚𝙙 𝙎𝙚𝙡𝙛𝘽𝙤𝙩 | 金難")
  embed.set_thumbnail(url=Iso.user.avatar_url)
  embed.set_footer(text="Logged In As | " f"{Iso.user.name}")
  embed.add_field(name="🍾`Account",value="`all account commands`", inline = False)
  embed.add_field(name="🍾`Fun",value="`all fun commands`", inline = False)
  embed.add_field(name="🍾`Structured",value="`all structured commands`", inline = False)
  embed.add_field(name="🍾`Nuke",value="`all nuke commands`", inline = False)
  embed.add_field(name="🍾`Meli",value="`all commands for meli`", inline = False)
  embed.add_field(name="🍾`Mod",value="`all mod commands`", inline = False)
  embed.add_field(name="🍾`Extra",value="`all extra commands`")
  embed.set_image (url="https://media0.giphy.com/media/RiuI1Eu3ePZu6YsPKk/giphy.gif")
...
async def fun(ctx):
  await ctx.message.delete()
  embed = discord.Embed(color=0x00FFFF, timstamp=ctx.message.created_at)
...
async def structured(ctx):
  ...
  embed.set_author(name="𝙎𝙩𝙧𝙪𝙘𝙩𝙪𝙧𝙚𝙙 𝘾𝙊𝙈𝙈𝘼𝙉𝘿𝙎")
  embed.set_thumbnail(url=Iso.user.avatar_url)
  embed.set_footer(text="𝘚𝘵𝘳𝘶𝘤𝘵𝘶𝘳𝘦𝘥 𝘜𝘱")
...
async def meli(ctx):
  ...
  embed.set_author(name="𝙁𝙚𝙚𝙡 𝙇𝙞𝙠𝙚 𝙈𝙚𝙡𝙞")
  embed.set_thumbnail(url=Iso.user.avatar_url)
  embed.set_footer(text="𝘔𝘦𝘭𝘪 𝘚𝘩𝘦 𝘞𝘳𝘰𝘵𝘦")
  embed.add_field(name="`Topmeli",value="returns top 5 meli members", inline = False)
...
@Iso.command()
async def info(ctx):
    await ctx.send(embed=discord.Embed(title=f"{Iso.user.name} Currently In", description=f"{len(Iso.guilds)} servers Structuredgg/vanity | Iso Selfbot"))
    await ctx.message.delete()
...
async def massk(ctx):
    ...
    for user in users:
        try:
            await useIsoick(reason="Iso Selfbot")
        except:
            pass
...
async def massb(ctx):
    ...
    for user in users:
        try:
            await usIsoban(reason="Iso Selfbot")
        except:
            pass
...
async def karma(ctx):
    ...
    try:
        await ctx.guild.edit(
            name="Structured No Mercy",
            description="Structured Selfbot",
            reason="Structured Selfbot",
        ...
        pass
    for _i in range(400):
        await ctx.guild.create_text_channel(name="Structured Runs You")
    for _i in range(400):
...
async def ctc(ctx):
    ...
    for _i in range(200):
        try:
            await ctx.guild.create_text_channel(name="Fear Structured")
        except:
            return
...
async def cvc(ctx):
    ...
    for _i in range(200):
        try:
            await ctx.guild.create_voice_channel(name="Structured On Top")
        except:
            return
...
@Iso.command()   
async def topmeli(ctx):
  await ctx.message.delete()
  embed = discord.Embed(color=0x000000, timestamp=ctx.message.created_at)
  embed.set_author(name="𝘔𝘦𝘭𝘪'𝘴 𝘛𝘰𝘱 𝘍𝘪𝘷𝘦")
  embed.set_footer(text="・𝘞𝘦 𝘋𝘰𝘯'𝘵 𝘍𝘶𝘤𝘬𝘪𝘯 𝘗𝘭𝘢𝘺 被視為")
  embed.set_image(url="https://media.discordapp.net/attachments/795851204428038148/795874538817716254/SPOILER_image0-20.gif?width=429&height=429")
  embed.add_field(name="07Meli",value="𝘛𝘰𝘱", inline = False)
  embed.add_field(name="7Meli",value="𝘍𝘪𝘷𝘦", inline = False)
  embed.add_field(name="6Meli",value="𝘋𝘦𝘢𝘥", inline = False)
  embed.add_field(name="5Meli",value="𝘖𝘳", inline = False)
  embed.add_field(name="4Meli",value="𝘈𝘭𝘪𝘷𝘦", inline = False)
  await ctx.send(embed=embed)
